...
run_analysis() {
    python scripts/convert_gqa_for_eval.py --src "$MERGE_FILE" --dst "$RESULT_DIR/$STAGE/testdev_balanced_predictions.json"
...
run_inference() {
    ...
    IDX=0
    for GPU_ID in "$@"; do
        CUDA_VISIBLE_DEVICES="$GPU_ID" python -m llava.eval.ModalPrompt.model_gqa \
